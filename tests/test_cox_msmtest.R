...
set.seed(50)
N=500
dat2 <- data.frame(stop=(tmg <- pmin(.1,rweibull(N, 10, 0.1))), 
                  start = pmax(0, tmg-runif(N)),
                  d=1.0*(tmg<0.1), x1=runif(N), x2=runif(N), z=runif(N))
expnms=paste0("x", 1:2)
f = survival::Surv(start,stop, d)~x1 + x2
f2 = survival::Surv(start,stop, d)~x1 + x2 + z
f3 = survival::Surv(start,stop, d)~x1 + x2 + I(z-mean(z))
m1 = survival::coxph(f, data = dat2)
m2 = survival::coxph(f2, data = dat2)
#(obj <- qgcomp.cox.noboot(f, expnms = expnms, data = dat))
#(obj2 <- qgcomp.cox.boot(f, expnms = expnms, data = dat, B=1000, MCiter=20000))
#(obj3 <- qgcomp.cox.boot(f2, expnms = expnms, data = dat, B=1000, MCiter=20000, parallel=TRUE))
...
head(sort(unique(dat2$stop*dat2$d))[-1])
head(times)
tail(sort(unique(dat2$stop*dat2$d))[-1])
tail(times)

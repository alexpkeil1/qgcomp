...
with a qgcomp.boot fit:
```{r logistic qgcompc, results='markup', fig.show='hold', fig.height=5, fig.width=7.5, cache=FALSE}
qcboot.fit2
```

with a qgcomp.boot fit, where the risk/prevalence ratio is estimated,  rather than the odds ratio:
...
while the `qgcomp.boot` result (MCsize=5000, B=500) yields a corrected standard error of 0.0398, which is much closer to the sandwich estimate of 0.0409 than the naive estimator (a second `qgcomp.boot` fit with fewer bootstrap iterations and smaller MCsize is included for display, but the more accurate results are commented out to reduce runtime). Here, the standard errors from the uncorrected fit are too low, but this may not always be the case.
...
future::plan(future::transparent) # return to standard evaluation
```

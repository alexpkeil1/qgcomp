...
```{r adjusting for covariates a, results='markup', fig.show='hold', fig.height=5, fig.width=7.5, cache=FALSE}
...
```{r adjusting for covariates b, results='markup', fig.show='hold', fig.height=5, fig.width=7.5, cache=FALSE}
...
```{r adjusting for covariates c, results='markup', fig.show='hold', fig.height=5, fig.width=7.5, cache=FALSE}
...
when exposure effects are non-linear (see below how to do this with `qgcomp.glm.boot`). 
...
```{r adjusting for covariates d, results='markup', fig.show='hold', fig.height=5, fig.width=7.5, cache=FALSE}
...
though this assumption can be relaxed via the 'degree' parameter in qgcomp.glm.boot, 
as follows:
...
```{r overall non-linearityb, results='markup', fig.show='hold', fig.height=5, fig.width=7.5, cache=FALSE}
...
```{r overall non-linearity psi interp, results='markup', fig.show='hold', fig.height=5, fig.width=7.5, cache=FALSE}
qcboot.fit5
```
...
```{r graphical non-linearity 1, results='markup', fig.show='hold', fig.height=3, fig.width=7.5, cache=FALSE}
library(splines)
...
```{r graphical non-linearity 1b, results='markup', fig.show='hold', fig.height=3, fig.width=7.5, cache=FALSE}
...
```{r graphical non-linearity 2, results='markup', fig.show='hold', fig.height=3, fig.width=7.5, cache=FALSE}
...
```{r graphical non-linearity 3, results='markup', fig.show='hold', fig.height=3, fig.width=7.5, cache=FALSE}
...
```{r graphical caution 2, results='markup', fig.show='hold', fig.height=3, fig.width=7.5, cache=FALSE}
...
```{r non-linear examples 2, results='markup', fig.show='hold', fig.height=3, fig.width=7.5, cache=FALSE}
...
```{r non-linear examples 3, results='markup', fig.show='hold', fig.height=3, fig.width=7.5, cache=FALSE}
...
```{r time-to-event3b, results='markup', fig.show='hold', fig.height=5, fig.width=7.5, cache=FALSE}
...
while the `qgcomp.glm.boot` result (MCsize=5000, B=500) yields a corrected standard error of 0.0398, which is much closer to the sandwich estimate of 0.0409 than the naive estimator (a second `qgcomp.glm.boot` fit with fewer bootstrap iterations and smaller MCsize is included for display, but the more accurate results are commented out to reduce runtime). Here, the standard errors from the uncorrected fit are too low, but this may not always be the case.
...
- The `qgcomp` package utilizes `multinom` from the `nnet` package in an analogous fashion to how `glm` is used. The joint effect estimates are interpreted as a ratio of the probability of being in the referent category of the outcome and the probability of being in the index category. Some (but not all) of the convenience functions, like `plot` and `summary` are available for multinomial fits, and more  functionality will be available in the future.
  
```{r mnom, results='markup', fig.show='hold', fig.height=10, fig.width=7.5, cache=FALSE}
...
# plot(rr) # This works, but it causes an issue in Windows with vignette building for this document
...
Alexandra J. White. A quantile-based g-computation approach to addressing the effects of exposure mixtures. <https://doi.org/10.1289/EHP5838> 

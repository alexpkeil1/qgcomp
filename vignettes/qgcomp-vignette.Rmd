...
# and a qgcomp.boot fit (B should be 200+, in practice):
gcompmod2 <- qgcomp.boot(disease_state~., expnms=Xnm, data = wqs_data[,c(Xnm, 'disease_state')], family=binomial(), q=4, B=10, seed=125)
...
gcompmod2 <- qgcomp.boot(y~sex+log_LBX074LA+log_LBX099LA+log_LBX105LA+log_LBX118LA+log_LBX138LA+log_LBX153LA+
                           ...
                           log_LBXF06LA+log_LBXF07LA+log_LBXF08LA+log_LBXF09LA+log_LBXPCBLA+log_LBXTCDLA+log_LBXHXCLA,
                         expnms=Xnm,
                         wqs_data, family=gaussian(), q=4, B=20, seed=125)
...
Compare with the `gwqs` output plots, which allows effects only in a single direction (the 'directional homogeneity' assumption)
...
Let's close with one more feature of `qgcomp` (and `qgcomp.boot`): handling non-linearity. Here is an example where we take the strongest (positive) predictor of the outcome in the primary model (log_LBXD02LA) and fit an indicator variable function by including the code `I(log_LBXD02LA==1) + I(log_LBXD02LA==2) + I(log_LBXD02LA==3)` in the formula.
...
gcompmod4 <- qgcomp(y~sex+
                           ...
                           log_LBXF06LA+log_LBXF07LA+log_LBXF08LA+log_LBXF09LA+log_LBXPCBLA+log_LBXTCDLA+log_LBXHXCLA,
                         expnms=Xnm,
                         wqs_data, family=gaussian(), q=4, B=20, degree=2, seed=125)
plot(gcompmod4)
```

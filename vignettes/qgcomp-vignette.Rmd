...
5. Using qgcomp to fit a time-to-event model to estimate conditional and marginal hazard ratios for the exposure mixture (coming soon)
...
```{r, results='markup', fig.show='hold', fig.height=5, fig.width=7.5, cache=FALSE}
...
which yields overall model confidence bounds, which represent the bounds that contain the true
regression line across 95% of studies, in expectation (provided correct model fit). The pointwise
bounds represent comparisons of the expected difference in outcomes at each quantile, with reference 
to a specific quantile (which can be specified by the user, as below). THe pointwise bounds will
generally give better agreement with statistical tests of model coefficients, as the model bounds
are not generally useful for determining whether mean outcomes at two specific levels of exposure
are statistically distinguishable at a given alpha level.
...
```{r, results='markup', fig.show='hold', fig.height=3, fig.width=7.5, cache=FALSE}
library(splines)
...
### Example 6: Miscellaneous other ways to allow non-linearity. Note that these are included as examples of *how* to include non-linearities, and are not intended as a demonstration of appropriate model selection.
```{r, results='markup', fig.show='hold', fig.height=3, fig.width=7.5, cache=FALSE}
